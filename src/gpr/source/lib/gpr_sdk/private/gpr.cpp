...
static void unpack_pixel_format(const gpr_buffer_auto *input_buffer, const gpr_parameters *convert_params,
                                gpr_buffer_auto *output_buffer) {
...
static void
set_vc5_encoder_parameters(vc5_encoder_parameters &vc5_encoder_params, const gpr_parameters *convert_params) {
...
void gpr_parameters_set_optimal(gpr_parameters *x) {
    ...
    x->enable_preview = false; // modified

    x->compute_md5sum = false;

    x->fast_encoding = true; // modified
}
    ...
    if (y->tuning_info.gain_map.size > 0) {
        for (int i = 0; i < 4; i++) {
            if (y->tuning_info.gain_map.buffers[i] != 0) {
                ...
                memcpy(x->tuning_info.gain_map.buffers[i], y->tuning_info.gain_map.buffers[i],
                       y->tuning_info.gain_map.size);
            }
        }
    }
}
...
static bool read_dng(const gpr_allocator *allocator,
...

    {
        ...
        if (gpmf_payload && gpmf_payload->LogicalSize() > 0) {
            if (convert_params != NULL && convert_params->gpmf_payload.buffer == NULL &&
                convert_params->gpmf_payload.size == 0) {
        ...
        if (negative->IsVc5Image(info)) {
            ...
            if (rawIFD.fOpcodeList2Count) {
                negative->OpcodeList2().Parse(host, *dng_read_stream, rawIFD.fOpcodeList2Count,
                                              rawIFD.fOpcodeList2Offset);
            }

            if (rawIFD.fOpcodeList3Count) {
                negative->OpcodeList3().Parse(host, *dng_read_stream, rawIFD.fOpcodeList3Count,
        ...
        if (convert_params) {
            ...
            // Copy ColorMatrix1, ColorMatrix2
            {
                ...
                memset(convert_params->profile_info.cam_to_srgb_1, 0,
                       sizeof(convert_params->profile_info.cam_to_srgb_1));
                memset(convert_params->profile_info.cam_to_srgb_2, 0,
                       sizeof(convert_params->profile_info.cam_to_srgb_2));
            }
            ...
            // Set Tuning Info
            {
...

                {
                    ...
                    bool rggb_raw = (rawIFD.fCFAPattern[0][0] == 0) && (rawIFD.fCFAPattern[0][1] == 1) &&
...
void calc_color_matrix(double in_matrix[3][3], double wb[3], double weight, double out_matrix[3][3]) {
    ...
    double sRGB_to_XYZd50[3][3] = {{0.4361, 0.3851, 0.1431},
...
static void write_dng(const gpr_allocator *allocator,
    ...
    { // Set Tuning Info
        ...
        if (gain_map_size > 0 && tuning_info->gain_map.buffers[0] != 0 && tuning_info->gain_map.buffers[1] != 0 &&
            tuning_info->gain_map.buffers[2] != 0 && tuning_info->gain_map.buffers[3] != 0) {
            dng_opcode_list &opcodelist2 = negative->OpcodeList2();
    ...
    // Set Bayer Pattern
    if (convert_params->tuning_info.pixel_format == PIXEL_FORMAT_RGGB_12 ||
        convert_params->tuning_info.pixel_format == PIXEL_FORMAT_RGGB_12P ||
        convert_params->tuning_info.pixel_format == PIXEL_FORMAT_RGGB_14) {
    ...
    if (convert_params->gpmf_payload.buffer != NULL && convert_params->gpmf_payload.size > 0) { // Set GPMF Data
        ...
        memcpy(gpmf_buffer_ptr + gopro_tag.Length(), convert_params->gpmf_payload.buffer,
               convert_params->gpmf_payload.size);
...
#if GPR_WRITING
    if (vc5_dng) {
        gpr_image_writer *gpr_writer = new gpr_image_writer(raw_image_buffer, convert_params->input_width,
                                                            convert_params->input_height, convert_params->input_pitch,
                                                            vc5_image_buffer);
        ...
        if (convert_params->enable_preview) {
            ...
            if (preview_image.jpg_preview.size > 0 && preview_image.jpg_preview.buffer != NULL) {
                ...
                memcpy(jpeg_preview->fCompressedData->Buffer_char(), preview_image.jpg_preview.buffer,
                       preview_image.jpg_preview.size);
                ...
                tje_encode_with_func(write_jpg_thumbnail, (void *) &context, 2, rgb_buffer.width, rgb_buffer.height, 3,
                                     (const unsigned char *) rgb_buffer.buffer);
...
bool gpr_convert_gpr_to_rgb(const gpr_allocator *allocator,
    ...
    if (vc5_decoder_process(&vc5_decoder_params, &vc5_buffer.get_gpr_buffer(), NULL, out_rgb_buffer) !=
        CODEC_ERROR_OKAY) {
        assert(0);
    }
